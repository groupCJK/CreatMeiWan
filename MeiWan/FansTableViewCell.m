...
- (void)setFansDic:(NSDictionary *)fansDic{
    ...
    [fansButton addTarget:self action:@selector(chlikFansButton) forControlEvents:UIControlEventTouchUpInside];
    fansButton.backgroundColor = [CorlorTransform colorWithHexString:@"#3f90a4"];
    [self addSubview:fansButton];
    self.fansButton = fansButton;
}
-(void)chlikFansButton{

    if (self.tappedBlock) {
        
        self.tappedBlock(self.fansDic,self.indexPath);
    }
}
